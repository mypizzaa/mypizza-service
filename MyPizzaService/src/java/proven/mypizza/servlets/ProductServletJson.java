...
public class ProductServletJson extends HttpServlet {
    ...
    /**
     * ProcessRequest executes actions sended by get to manage categories.
     *
     * @param request
     * @param response
     */
        ...
        if (action != null) {
            switch (action) {
                case "list_all_pizzas": //list all pizzas
                    ...
                    break;
                case "list_all_ingredients": //list all ingredients
                    listAllIngredients(request, response);
                    break;
                case "list_all_drinks": //list all ingredients
                    listAllDrinks(request, response);
                    break;
                case "list_ingredients_by_pizza":
                    listIngredientsByPizzaId(request, response);
                    break;
            }
    ...
    /**
     * ProcessRequest executes actions sended by post to manage categories.
     *
     * @param request
     * @param response
     */
    ...
    private void listIngredientsByPizzaId(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
          MyPizzaRequestResult result = null;
        try {
            long id = Long.parseLong(request.getParameter("id"));
            List<Ingrediente> entityList = (List<Ingrediente>) model.getIngredientsFromPizzaId(id);
             result = new MyPizzaRequestResult(entityList, 1);            
        }catch(NumberFormatException nfe)  {
              result = new MyPizzaRequestResult("Invalid parameter", 0); 
        } 
        request.setAttribute("result", result);
        RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/jsp/json-result.jsp");
        rd.forward(request, response);
    ...
    private void listAllIngredients(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        List<Ingrediente> entityList = (List<Ingrediente>) model.getAllIngredients();
        MyPizzaRequestResult result = new MyPizzaRequestResult(entityList, 1);
        request.setAttribute("result", result);
        RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/jsp/json-result.jsp");
        rd.forward(request, response);
    ...
    private void listAllDrinks(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        List<Refresco> entityList = (List<Refresco>) model.getAllDrinks();
        MyPizzaRequestResult result = new MyPizzaRequestResult(entityList, 1);
        request.setAttribute("result", result);
        RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/jsp/json-result.jsp");
        rd.forward(request, response);
